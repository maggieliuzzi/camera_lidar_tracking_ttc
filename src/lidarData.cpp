...
{
    /* Removes LiDAR points based on min and max distance in X, Y and Z */
    
    std::vector<LidarPoint> newLidarPts; 
    for (auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
       if ((*it).x>=minX && (*it).x<=maxX && (*it).z>=minZ && (*it).z<=maxZ && (*it).z<=0.0 && abs((*it).y)<=maxY && (*it).r>=minR) {  // checking if Lidar point is outside of boundaries
...
{
    /* Loads LiDAR points from a given location and stores them in a vector */
...
void showLidarTopview(std::vector<LidarPoint> &lidarPoints, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    // plotting LiDAR points into image
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it) {
...
{
    // initialising image for visualisation
    cv::Mat visImg; 
    ...
    // returning augmented image or waiting if no image has been provided
    if (extVisImg == nullptr) {
